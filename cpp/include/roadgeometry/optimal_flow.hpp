...
struct FlowInstance {
    ...
    std::vector<PiecewiseLinear>       edge_cost;  // indexed by edge_id
    double                             U;
};
...
FlowReduction<Road, Vertex> build_flow_reduction(
    ...
    for (auto& [road, length] : lengths) {
        ...
        PiecewiseLinear obj = objective_from_measure(m);
